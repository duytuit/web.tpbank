...
class User extends Authenticatable implements JWTSubject
{
    ...
    public function scopeFilterByEmail($query, $email = null)
    {
     ...
     /**
     * Scope a query to only include users with the given phone or email.
     *
     * @param \Illuminate\Database\Eloquent\Builder $query
     * @param string $phone
     *
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function scopeFilterByMailOrPhone($query, $account = null)
    {
        if (!$account) {
            return $query;
        }

        return $query->where('email', 'like', '%' . $account . '%')->orWhere('phone', 'like', '%' . $account . '%');
    }

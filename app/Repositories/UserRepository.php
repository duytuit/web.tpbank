...
class UserRepository
{
     ...
     * @return User
     */
    public function findByEmailOrPhone($account = null)
    {
        return $this->user->with('profile', 'roles')->FilterByMailOrPhone($account)->first();
    }
     ...
     * @return User
     */
    public function findByEmail_v2($email = null)
    {
    ...
    {
        return $this->user->with('profile')->FilterById($id)->first();
    }
    ...
    {
        return $this->user->FilterByMailOrPhone($phone)->first();
    }
    ...
    {
        return $this->user->Status()->where('code', '=', $code)->first();
    }
    ...
    { 
        try {
            ...
            if ($e->errorInfo[1] === 1062) {
                throw ValidationException::withMessages(['message' => trans('user.user_duplicate')]);
            } else {
                throw $e;
            }
        }
     ...
     * @return User
     */
    public function createOtp($params, $register = 0)
    { 
        try {
            $user = $this->user->forceCreate($this->formatParamsOtp($params, 'register'));
        } catch (QueryException $e) {
            if ($e->errorInfo[1] === 1062) {
                throw ValidationException::withMessages(['message' => trans('user.user_duplicate')]);
            } else {
    ...
    private function formatParams($params, $action = 'create')
    {
        ...
        $formatted = [
            'email' => isset($params['email']) ? $params['email'] : $email ?? null,
            'phone' => isset($params['phone']) ? $params['phone'] : $phone ?? null,
            'status' => 'activated',
     ...
     * @return array
     */
    private function formatParamsOtp($params, $action = 'create')
    {
        if(isset($params['account'])){
            if(filter_var($params['account'], FILTER_VALIDATE_EMAIL) ) {
                $email = $params['account'];

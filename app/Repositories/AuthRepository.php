...
class AuthRepository
{
    ...
    public function auth($params = array())
    {
        ...
        if(isset($params['account'])){
                if(filter_var($params['account'], FILTER_VALIDATE_EMAIL) ) {
                    ...
                    $auth_user = $this->user->findByEmailOrPhone($email);
                }else{
                    $phone = $params['account'];
                    $auth_user = $this->user->findByEmailOrPhone($phone);
                }
        }else{
     ...
     * @throws ValidationException
     */
    public function validateUserAndStatusForResetPasswordAPI($phone = null)
    {
        $user = $this->user->findByPhone($phone);

        if (!$user) {
            throw ValidationException::withMessages(['phone' => trans('passwords.user')]);
        }
        if ($user->status != 'activated') {
            throw ValidationException::withMessages(['phone' => trans('passwords.account_not_activated')]);
        }
     ...
     * @throws ValidationException
     */
    public function passwordApi($params = [])
    {
        $this->validateResetPasswordStatus();
        if(filter_var($params['account'], FILTER_VALIDATE_EMAIL) ) {
            $user = $this->validateUserAndStatusForResetPassword($params['account']);
        }else{
            $user = $this->validateUserAndStatusForResetPasswordAPI($params['account']);
        }
        $token = Str::uuid();
        \DB::table('password_resets')->insert([
            'email' => $params['account'],
     ...
     * @throws ValidationException
     */
    public function resetAPI($params = [])
    {
        $this->validateResetPasswordStatus();

        $user = $this->validateUserAndStatusForResetPasswordAPI($params['phone']);

        $this->validateResetPasswordToken($params['token'], $params['email'] ?? null);
